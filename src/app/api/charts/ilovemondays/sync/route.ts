...
import { performILoveMondaysSync } from "@/lib/ilovemondays-sync";
...
export async function POST(req: NextRequest) {
  ...
  const { data: userRow } = await supabase
    .from("users")
    .select("user_id, spotify_access_token, spotify_refresh_token, lastfm_session_key, lastfm_username, ilovemondays_playlist_id, ilovemondays_playlist_name")
  ...
  const effectivePlaylistId = (playlistId as string | undefined) ?? (userRow.ilovemondays_playlist_id as string | undefined) ?? undefined;
  const effectivePlaylistName = (playlistName as string | undefined) ?? (userRow.ilovemondays_playlist_name as string | undefined) ?? undefined;

  async function doSync(accessToken: string) {
    const result = await performILoveMondaysSync({
    ...
    const hadPref = !!(userRow!.ilovemondays_playlist_id || userRow!.ilovemondays_playlist_name);
    ...
    if (shouldSave) {
      await supabaseAdmin
        .from("users")
        .update({ ilovemondays_playlist_id: result.playlistId, ilovemondays_playlist_name: effectivePlaylistName ?? null, updated_at: new Date().toISOString() })
        .eq("user_id", userId);
    }
  ...
  try {
    ...
    try {
      ...
      await supabaseAdmin.from("events").insert({ user_id: userId, event_type: "ilovemondays_sync", status: "fail", payload: { error: e2?.message ?? "sync_failed" } });
