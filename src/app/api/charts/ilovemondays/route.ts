...
export async function GET(req: NextRequest) {
  ...
  const { data: user } = await supabase
    .from("users")
    .select("user_id, spotify_access_token, ilovemondays_playlist_id, ilovemondays_playlist_name")
    .eq("spotify_user_id", ownerSpotifyId)
    .single();
  if (!user?.user_id) return Response.json({ tracks: [], updatedAt: null }, { status: 200 });

  const { data: snap } = await supabase
    .from("charts_ilovemondays")
    .select("track_ids, generated_at, week_start_date")
    .eq("user_id", user.user_id)
    .single();

  let target: any = { id: user.ilovemondays_playlist_id ?? null, name: user.ilovemondays_playlist_name ?? null };
  if (user.ilovemondays_playlist_id && user.spotify_access_token) {
    try {
      const meta = await SpotifyAPI.getPlaylistMeta({ access_token: user.spotify_access_token }, user.ilovemondays_playlist_id);
  ...
  return Response.json({
    ...
    weekStartDate: (snap?.week_start_date ?? null) as string | null,
    target,
  });
}
