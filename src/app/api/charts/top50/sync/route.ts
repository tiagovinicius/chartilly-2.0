...
export async function POST(req: NextRequest) {
  ...
  const effectivePlaylistId = (playlistId as string | undefined) ?? (userRow.top100_playlist_id as string | undefined) ?? undefined;
  const effectivePlaylistName = (playlistName as string | undefined) ?? (userRow.top100_playlist_name as string | undefined) ?? undefined;
  ...
  async function doSync(accessToken: string) {
    ...
    const hadPref = !!(userRow!.top100_playlist_id || userRow!.top100_playlist_name);
    ...
    if (shouldSave) {
      await supabaseAdmin
        .from("users")
        .update({ top100_playlist_id: result.playlistId, top100_playlist_name: effectivePlaylistName ?? null, updated_at: new Date().toISOString() })
        .eq("user_id", userId);
    }
